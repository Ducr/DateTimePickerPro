...
export default {
  ...
  props: {
    ...
    customMinuteStep: { // 分钟数自定义步距，若大于59，则只显示0
      type: [String, Number],
      default: 1
    },
    customSecondStep: { // 秒钟数自定义步距，若大于59，则只显示0
      type: [String, Number],
      default: 1
    }
  },
  ...
  methods: {
    ...
    handleStartChange(event) {
      ...
      if (value) {
        ...
        this.handleValidateTimeNumAndFormat('startTime');
      }
    },
    ...
    handleEndChange(event) {
      ...
      if (value) {
        ...
        this.handleValidateTimeNumAndFormat('endTime');
      }
    },
    ...
    /**
     * @description 表单输入框失焦时，对分钟、秒钟数进行合法化处理
     * @author Ducr
     */
    handleBlur(type) {
      const value = this.parseString(this.displayValue);
     ...
     * @author Ducr
     */
    handleValidateTimeNumAndFormat(type) {
      this.$nextTick(() => {
        // 编辑非时间范围的输入框
        if (!this.ranged) {
          ...
          const timeArr = this.parseString(this.displayValue);
          if (type === 'startTime') {
            if (this.picker && this.picker.$children && this.picker.$children.length) {
    ...
    mountPicker() {
      ...
      this.picker.customMinuteStep = this.customMinuteStep;
      this.picker.customSecondStep = this.customSecondStep;
