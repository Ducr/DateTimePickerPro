            ...
            <span class="el-date-range-picker__editors-wrap">
              ...
              <span class="el-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                <el-input
                  ...
                  @blur="handleTimeBlur('min')"
                  @focus="minTimePickerVisible = true; handleTimeFocus('min')"
            ...
            <span class="el-date-range-picker__editors-wrap is-right">
              ...
              <span class="el-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                <el-input
                  ...
                  :readonly="!minDate"
                  @blur="handleTimeBlur('max')"
                  @focus="(minDate && (maxTimePickerVisible = true)); handleTimeFocus('max')"
  ...
  export default {
    ...
    data() {
      return {
        ...
        customMinuteStep: 30, // 分钟数自定义步距，用于pick.vue中修改
        customSecondStep: 1, // 秒钟数自定义步距，用于pick.vue中修改
      };
    },
    ...
    methods: {
       ...
       * @author Ducr
       * @param { String } pickerType 实例名的key，minTimePicker还是maxTimePicker
       * @param { String } dateKey 时间值的key，minDate还是maxDate
       */
      handleValidateTimeNumAndFormat(pickerType, dateKey) {
        if (this.$refs[pickerType] && this.$refs[pickerType].$children && this.$refs[pickerType].$children.length) {
          const { $children = [] } = this.$refs[pickerType];
          const [ timeSpinnerVm = {} ] = $children;
      ...
      /**
       * @description 时分秒输入框的失焦事件，进行合法化处理分钟数
       * @author Ducr
       * @param { String } type 起始和结束时间的输入框类型，min为起始，max为结束
       */
      handleTimeBlur(type) {
        this.$nextTick(() => {
          if (type === 'min') {
            this.handleValidateTimeNumAndFormat('minTimePicker', 'minDate');
            this.maxDate && this.handleValidateTimeNumAndFormat('maxTimePicker', 'maxDate');
          } else {
            this.maxDate && this.handleValidateTimeNumAndFormat('maxTimePicker', 'maxDate');
          }
        });
      ...
      /**
       * @description 时分秒输入框的聚焦事件，进行合法化处理分钟数
       * @author Ducr
       * @param { String } type 起始和结束时间的输入框类型，min为起始，max为结束
       */
...
// 防止引用后组件后，被初始化样式干扰到picker的时间输入框
.el-picker-panel {
