        ...
        <div class="el-picker-panel__body">
          <div class="el-date-picker__time-header" v-if="showTime">
            ...
            <span class="el-date-picker__editor-wrap" v-clickoutside="handleTimePickClose">
              <el-input
                ref="input"
                @focus="timePickerVisible = true; handleTimeFocus()"
    ...
    methods: {
      ...
      changeToNow() {
        ...
        if ((!this.disabledDate || !this.disabledDate(new Date())) && this.checkDateWithinRange(new Date())) {
          this.date = new Date();
          this.emit(this.date);
          this.handleValidateTimeNumAndFormat();
        }
      },
       ...
       * @author Ducr
       */
      handleValidateTimeNumAndFormat() {
        if (this.$refs.timepicker && this.$refs.timepicker.$children && this.$refs.timepicker.$children.length) {
          const { $children = [] } = this.$refs.timepicker;
          const [ timeSpinnerVm = {} ] = $children;
          const { minutesList = [], secondsList = [], queryApproximate = ((val, list = []) => val) } = timeSpinnerVm;
          const currDate = this.date ? this.date : new Date();
          ...
          this.$refs.timepicker.value = parsedDate;
          this.date = parsedDate;
          // this.emit(parsedDate);
          // 使用下面这个带true的，否则点击时分秒滚动面板会被自动关闭
          this.emit(parsedDate, true);
          this.userInputTime = null;
        }
      },
