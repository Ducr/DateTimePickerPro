...
<script type="text/babel">
  import { limitTimeRange, isDate, clearMilliseconds, timeWithinRange, formatDate } from '../../utils/date-util';
  ...
  export default {
    ...
    methods: {
      ...
      handleConfirm(visible = false, first) {
        if (first) return;
        let date = clearMilliseconds(limitTimeRange(this.date, this.selectableRange, this.format));
        // Ducr：点击时分秒面板确认按钮时，进行合法化处理
        if (this.$children && this.$children.length) {
          const { $children = [] } = this;
          const { minutesList = [], secondsList = [], queryApproximate = ((val, list = []) => val) } = $children[0] || {};
